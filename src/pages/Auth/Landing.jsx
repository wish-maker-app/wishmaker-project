...
export default function Landing() {
  ...
  return (
    <AuthShell>
      {/* Wrapper contenu : mobile garde son ambiance bleue, desktop blanc */}
      <div className="min-h-screen flex flex-col bg-[#5B6BF5] lg:bg-white">
        <div className="mx-auto w-full max-w-[480px] flex flex-col flex-1 lg:max-w-[460px] lg:px-2">

          {/* Partie haute : titre + sous-titre */}
          <motion.div
            initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }}
            className="flex flex-col items-center justify-center px-6 py-16 lg:py-12 gap-3 text-center"
          >
            <h1 className="font-bold text-3xl lg:text-[34px] tracking-[-0.02em] leading-tight text-white lg:text-[#1A1A2E]">
              {t('auth.register_email.titre')}
            </h1>
            <p className="text-sm leading-relaxed text-white/80 lg:text-[#3A3A4E] max-w-[360px]">
              {t('auth.register_email.sous_titre')}
            </p>
          </motion.div>

          {/* Carte formulaire : blanche montante sur mobile, plate sur desktop */}
          <motion.div
            initial={{ y: 60, opacity: 0 }} animate={{ y: 0, opacity: 1 }}
            transition={{ duration: 0.4, delay: 0.15 }}
            className="flex-1 bg-white px-6 pt-6 lg:pt-2 pb-8 flex flex-col gap-4 lg:bg-transparent lg:flex-none"
